...
// Upload to SnapRec (R2)
async function uploadToSnapRec() {
        ...
        if (result && result.success) {
            ...
            if (result.shareUrl) {
                try {
                    ...
                    // Open the website to view the recording with claim parameter
                    window.open(result.shareUrl + '?claim=true', '_blank');
                } catch (e) {
...
function setupEventListeners() {
    ...
    // SnapRec upload button
    const driveBtn = document.getElementById('driveBtn');
    if (driveBtn) {
        driveBtn.addEventListener('click', uploadToSnapRec);
    }
